...
@onready var _resshan := $MarginContainer/Resshan

func _ready() -> void:
	set_process(false)
	_resshan.set_drag_forwarding(_get_entry_drag_data, _can_drop_entry, _drop_entry)
	player_input.set_drag_forwarding(_get_no_drag_data, _can_drop_entry, _drop_entry)
	

func _process(_delta: float) -> void:
	if is_instance_valid(_drag_preview):
		_drag_preview.position.x = -drag_offset
...
func _draw() -> void:
	...
	var shape: = LanguageRenderer.draw_resshan_text(
		_resshan.position + Vector2(0, 75), resshan_string, self, true,
		80, Color.BLACK
	)
	_resshan.custom_minimum_size.x = shape.size.x
	_resshan.custom_minimum_size.y = shape.size.y
...
func _get_entry_drag_data(_position: Vector2) -> Variant:
	print("get entry drag data")
	var preview := duplicate() as NotebookEntry
	var holder := DragPreview.new()
...
func _on_resshan_mouse_entered() -> void:
	Input.set_custom_mouse_cursor(open_hand, Input.CURSOR_ARROW, Vector2(16, 16) )
